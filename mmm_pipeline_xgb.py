...
# Step 3: Model Fitting using XGBoost
import xgboost as xgb
from sklearn.preprocessing import StandardScaler
...
def update_graph(*spend_values):
    ...
    predicted_sales = model.predict(X_new)[0]*12

    # Estimate ROI as (predicted_sales - total_spend) / total_spend
    roi = ((predicted_sales - total_spend) / total_spend) if total_spend > 0 else 0

    # Estimate prediction confidence using std dev of cross-val
    import scipy.stats as st
    pred_std = rmse_scores.mean()
    conf_int = st.norm.interval(0.95, loc=predicted_sales, scale=pred_std)
    ...
    fig.add_trace(go.Scatter(
        x=[predicted_time, predicted_time],
    ...
    fig.update_layout(
        title='Total Sales Trend with Dynamic Prediction and Confidence Interval',
        xaxis_title='Time',
        yaxis_title='Sales',
        showlegend=True,
    ...
    predicted_text = f"Predicted Sales: {predicted_sales:,.2f} | 95% CI: ({conf_int[0]:,.2f} - {conf_int[1]:,.2f}) | ROI: {roi*100:.1f}%"
